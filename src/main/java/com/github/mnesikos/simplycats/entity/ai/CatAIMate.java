...
public class CatAIMate extends Goal {
    ...
    public boolean shouldExecute() {
        ...
        this.nearbyCats = world.getEntitiesWithinAABB(this.cat.getClass(), this.cat.getBoundingBox().grow(NEARBY_RADIUS_CHECK));
        if (this.nearbyCats.size() >= SimplyCatsConfig.BREEDING_LIMIT.get())
            return false;
    ...
    public boolean shouldContinueExecuting() {
        ...
        return maleCooldownCheck && this.target.isAlive() && femaleHeatCheck && this.mateDelay < 60
                && this.nearbyCats.size() < SimplyCatsConfig.BREEDING_LIMIT.get() && this.cat.getEntitySenses().canSee(this.target);
    }

    private boolean ownerExceedsLimit(EntityCat tamedCat, LivingEntity owner) {
        return tamedCat != null && tamedCat.isTamed() && SimplyCatsConfig.TAMED_LIMIT.get() != 0 && owner != null && owner.getPersistentData().getInt("CatCount") >= SimplyCatsConfig.TAMED_LIMIT.get();
    }

    private boolean ownerIsOffline(EntityCat tamedCat, LivingEntity owner) {
    ...
    private EntityCat getNearbyMate() {
        World world = this.cat.world;
        List<EntityCat> list = world.getEntitiesWithinAABB(this.cat.getClass(), this.cat.getBoundingBox().grow(NEARBY_RADIUS_CHECK));
