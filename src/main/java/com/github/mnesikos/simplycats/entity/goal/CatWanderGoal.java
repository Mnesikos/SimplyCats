...
public class CatWanderGoal extends Goal {
    ...
    private Vec3 getPosition() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (blockPos3.getY() >= 0 && blockPos3.getY() <= cat.level.getMaxBuildHeight() && (!outsideBounds || (cat.getHomePos().get().distSqr(blockPos3) < (SCConfig.wander_area_limit.get() * SCConfig.wander_area_limit.get()))) && pathNavigator.isStableDestination(blockPos3)) {
