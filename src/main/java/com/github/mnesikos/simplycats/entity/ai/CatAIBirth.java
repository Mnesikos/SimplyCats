...
public class CatAIBirth extends EntityAIBase {
    ...
    private void spawnBaby(EntityCat father) {
        ...
        if (child != null) {
            if (this.MOTHER.getOwner() != null) {
                EntityPlayer owner = (EntityPlayer) this.MOTHER.getOwner();
                owner.triggerAchievement(StatList.field_151186_x);
            }
            ...
            for (int i = 0; i < 7; ++i) {
                ...
                this.WORLD.spawnParticle("heart", this.MOTHER.posX + (double) (random.nextFloat() * this.MOTHER.width * 2.0F) - (double) this.MOTHER.width, this.MOTHER.posY + 0.5D + (double) (random.nextFloat() * this.MOTHER.height), this.MOTHER.posZ + (double) (random.nextFloat() * this.MOTHER.width * 2.0F) - (double) this.MOTHER.width, d0, d1, d2);
            }

            if (this.WORLD.getGameRules().getGameRuleBooleanValue("doMobLoot")) {
                this.WORLD.spawnEntityInWorld(new EntityXPOrb(this.WORLD, this.MOTHER.posX, this.MOTHER.posY, this.MOTHER.posZ, random.nextInt(2) + 1));
            }
        }
    }
}
