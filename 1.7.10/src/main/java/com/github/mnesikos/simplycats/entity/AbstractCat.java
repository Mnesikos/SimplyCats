...
public abstract class AbstractCat extends EntityTameable {
    ...
    public void onUpdate() {
        ...
        /*if (this.getAttackTarget() == null) {
            List<EntityZombie> zombies = this.worldObj.getEntitiesWithinAABB(EntityZombie.class, this.getBoundingBox().expand(4.0D, 3.0D, 4.0D));
            List<EntitySkeleton> skeletons = this.worldObj.getEntitiesWithinAABB(EntitySkeleton.class, this.getBoundingBox().expand(4.0D, 3.0D, 4.0D));
            if ((!zombies.isEmpty() || !skeletons.isEmpty()) && this.rand.nextInt(400) == 0) {
                this.playSound("mob.cat.hiss", this.getSoundVolume() / 2.0F, this.getSoundPitch());
            }
        }*/
